...
export default function InfoLogin({ form, setForm, setEtapa }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (camposVazios) return setErroGeral("Preencha todos os campos.");
    if (!emailValido) return setErroGeral("Digite um email válido.");
    if (!senhaValida) return setErroGeral("A senha não atende aos requisitos.");
    if (!confirmacaoOk) return setErroGeral("As senhas não coincidem.");

    // ✅ Atualiza o form com email também
    setForm(prev => ({ ...prev, email }));
    setErroGeral("");
    setEtapa(1); // Avança para a tela ValidaEmail (que será ignorada, só botão de próximo)
  };
